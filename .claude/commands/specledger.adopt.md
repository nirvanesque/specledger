...
- **Audit mode**: Use pre-analyzed module data from `/specledger.audit-deep`
...
4. If stale: ERROR "Run /specledger.audit-deep --force"
...
2. Find highest spec number: check `specs/[0-9]+-*` directories
3. Run branch creation script:
   ```bash
   .specify/scripts/bash/adopt-feature-branch.sh --json --number N+1 --short-name "name" "description"
   ```
...
Apply validation from `.specify/templates/partials/spec-quality-validation.md`:
...
- **Stale audit**: "Audit cache stale - run /specledger.audit-deep --force"
